<script setup>
// Wikipedia article reader: renders sanitized article HTML and intercepts
// link clicks — internal /wiki/<X> links navigate inside Safari, external
// links open in a real browser tab.
...
function onArticleClick(e) {
  ...
  if (internal) {
    ...
    if (page && !page.includes(':')) {
      ...
      window.open('https://en.wikipedia.org/wiki/' + encodeURIComponent(page), '_blank') // File:/Special:/etc.
    }
...
<style scoped>
.reader-wrap { flex: 1; overflow-y: auto; background: var(--window-bg); }
.reader-loading { display: flex; align-items: center; justify-content: center; gap: 8px; height: 100%; color: var(--text-dim); font-size: 13px; }
.spin {
  width: 14px;
  height: 14px;
  border-radius: 50%;
...
.reader-source { font-size: 12px; color: var(--text-dim); margin: 6px 0 18px; }

/* Article content (injected HTML — needs :deep) */
.reader-body :deep(h2) {
  font-family: Georgia, 'Times New Roman', serif;
  font-size: 21px;
  font-weight: 500;
...
.reader-body :deep(a) { color: var(--accent); text-decoration: none; cursor: default; }
...
.reader-body :deep(th), .reader-body :deep(td) { padding: 4px 8px; border: 0.5px solid var(--border); }
...
.reader-body :deep(.thumbcaption) { font-size: 11.5px; color: var(--text-dim); padding: 4px 2px; }
...
.reader-body :deep(.hatnote) { font-size: 12px; color: var(--text-dim); font-style: italic; }
.reader-body :deep(.reflist) { font-size: 11.5px; color: var(--text-dim); }
.reader-body :deep(sup.reference) { font-size: 10px; }
.reader-body :deep(.navbox), .reader-body :deep(.metadata) { font-size: 12px; background: var(--sidebar-bg); }
